...
static INT_PTR WriteContactSetting(WPARAM wParam,LPARAM lParam)
{
	...
	if(ofsSettingsGroup==0) {  //module group didn't exist - make it
		if(dbcws->value.type&DBVTF_VARIABLELENGTH) {
 		  if(TYPE_IS_STRING(dbcws->value.type)) bytesRequired=(int)strlen(dbcws->value.pszVal)+2;
			...
			NeedBytes(3);
 			if(pBlob[0]!=dbcws->value.type || (TYPE_IS_STRING(pBlob[0]) && *(PWORD)(pBlob+1)!=strlen(dbcws->value.pszVal)) || (pBlob[0]==DBVT_BLOB && *(PWORD)(pBlob+1)!=dbcws->value.cpbVal)) {
				...
				switch(dbcws->value.type) {
					...
					default: if (TYPE_IS_STRING( dbcws->value.type ))
								DBWrite(ofsBlobPtr+2,dbcws->value.pszVal,(int)strlen(dbcws->value.pszVal)); break;
