...
#pragma pack(pop)
	...
	public:
		...
		protected:
			...
			PListItem m_Item;
			hash_multimap<TKey, TData, FHash> * m_Owner;

			iterator(hash_multimap<TKey, TData, FHash> * Owner, PListItem Item)
				: m_Owner(Owner)
			{
				m_Owner->addRef();
				m_Item = Item;
	...
	template <typename TKey, typename TData, uint32_t (*FHash)(const void *, uint32_t)>
	bool hash_multimap<TKey, TData, FHash>::listFind(const PListItem BucketNode, const uint32_t Hash, const TKey & Key, const PListItem Node, PListItem * & Prev, PListItem & Curr, PListItem & Next)
	{
