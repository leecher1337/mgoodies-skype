...
int IEViewOptInit(WPARAM wParam, LPARAM lParam)
{
	...
	for (i = 1; i < SIZEOF(tabPages); i++) {
		...
		odp.ptszTab = (TCHAR*)tabPages[i].tabName;
		CallService(MS_OPT_ADDPAGE, wParam, (LPARAM) & odp);
	}
	return 0;
}
	...
	case UM_CHECKSTATECHANGE:
		{
			...
			if (proto != NULL) {
				if (strcmpi(proto->getProtocolName(), "_default_")) {
					proto->setSRMMEnableTemp(TreeView_GetCheckState((HWND)wParam, (HTREEITEM) lParam) != 0);
				}
			}
	...
	case UM_CHECKSTATECHANGE:
		{
			...
			if (proto != NULL) {
				if (strcmpi(proto->getProtocolName(), "_default_")) {
					proto->setHistoryEnableTemp(TreeView_GetCheckState((HWND)wParam, (HTREEITEM) lParam) != 0);
				}
			}
			if ((HTREEITEM) lParam != TreeView_GetSelection((HWND)wParam)) {
				TreeView_SelectItem((HWND)wParam, (HTREEITEM) lParam);
	...
	case UM_CHECKSTATECHANGE:
		{
			...
			if (proto != NULL) {
				if (strcmpi(proto->getProtocolName(), "_default_")) {
					proto->setChatEnableTemp(TreeView_GetCheckState((HWND)wParam, (HTREEITEM) lParam) != 0);
				}
			}
			if ((HTREEITEM) lParam != TreeView_GetSelection((HWND)wParam)) {
				TreeView_SelectItem((HWND)wParam, (HTREEITEM) lParam);
...
void Options::init() {
	...
	for (int i = 0; i < protoCount+1; i++) {
		...
		proto->setSRMMEnable(i==0 || DBGetContactSettingByte(NULL, ieviewModuleName, dbsName, FALSE));
		...
		proto->setChatEnable(i==0 || DBGetContactSettingByte(NULL, ieviewModuleName, dbsName, FALSE));
		...
		proto->setHistoryEnable(i==0 || DBGetContactSettingByte(NULL, ieviewModuleName, dbsName, FALSE));
	...
	bMathModule = ServiceExists(MTH_GET_GIF_UNICODE) != 0;
	bSmileyAdd = ServiceExists(MS_SMILEYADD_BATCHPARSE) != 0;
