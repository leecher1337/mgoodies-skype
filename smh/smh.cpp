...
#define DEFAULT_TEMPLATE_REMOVE Translate("removed his/her status message")
...
void AddToHistory(HANDLE hContact, char *nickname)
{
	...
	char log[1024] = "";
	mir_snprintf(log, sizeof(log), templ, nickname == NULL ? Translate("<no status message>") : nickname);
...
void AddToHistory(HANDLE hContact, wchar_t *nickname)
{
	...
	// Replace template with nick
	wchar_t log[1024] = L"";
	mir_sntprintf(log, sizeof(log), wtempl, nickname == NULL ? TranslateT("<no status message>") : nickname);
