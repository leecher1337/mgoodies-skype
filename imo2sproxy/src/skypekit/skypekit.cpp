...
public:
  ...
  MyContact(unsigned int oid, MySkype* root) : Contact(oid, root) { this->root = root; };
...
public:
  ...
  MyContactGroup(unsigned int oid, MySkype* root) : ContactGroup(oid, root) { this->root = root; };
...
public:
    ...
    MyAccount(unsigned int oid, MySkype* root) : Account(oid, root) { this->root = root; loggedIn=false; loggedOut=false; };
    void OnChange(int prop);
...
public:
	...
	MyParticipant(unsigned int oid, MySkype* root) : Participant(oid, root) { this->root = root; }
	void OnChange(int prop);
private:
	bool DoChange(int prop);
	MySkype *root;
...
public:
	...
	MyConversation(unsigned int oid, MySkype* root) : Conversation(oid, root) { this->root = root; }
	void OnChange(int prop);
	bool DoChange(int prop, bool bError = true);
private:
	MySkype *root;
...
MyContactSearch::MyContactSearch(unsigned int oid, MySkype* root) : ContactSearch(oid, root)
{ 
	this->root = root; 
}
...
int Imo2S_Login (IMOSAPI *pInst, char *pszUser, char *pszPass, char **ppszError)
{
	...
	MyAccount::Ref account;
	if (pInst->s->GetAccount(pInst->pszUser, account))
	{
        Account::STATUS LoginStatus;
		pInst->account = account;
		pInst->account->LoginWithPassword(pInst->pszPass, false, true);
		while ( (!pInst->account->loggedIn) && (!pInst->account->loggedOut) ) { Delay(1); }; 
		pInst->account->GetPropStatus(LoginStatus);
        if (LoginStatus == Account::LOGGED_IN)
		{
			...
			pInst->iLoginStat = 1;
			pInst->skypeNamesContactGroup = skypeNamesContactGroup;
			pInst->s->GetHardwiredContactGroup(ContactGroup::SKYPE_BUDDIES, pInst->skypeNamesContactGroup);
			pInst->skypeNamesContactGroup->GetContacts(pInst->skypeNamesContactGroup->contactList);
			fetch(pInst->skypeNamesContactGroup->contactList);
			for (i = 0; i < pInst->skypeNamesContactGroup->contactList.size(); i++)
			{
				SEString contactName, moodText;
				
				Contact::AVAILABILITY avail;

				if (!pInst->skypeNamesContactGroup->contactList[i]->GetPropSkypename(contactName))
					continue;
				
				if (pInst->skypeNamesContactGroup->contactList[i]->GetPropAvailability(avail))
				{
				...
				if (pInst->skypeNamesContactGroup->contactList[i]->GetPropMoodText(moodText))
...
void Imo2S_Logout(IMOSAPI *pInst)
{
	...
	pInst->account->Logout(false);
	while (!pInst->account->loggedOut) { Delay(1); };
	pInst->iLoginStat = 0;
	pInst->account->loggedIn = false;
	pInst->account->loggedOut = false;
	Send (pInst, "CONNSTATUS OFFLINE");
}
...
bool MyAccount::DoChange(int prop)
{
	switch (prop)
	{
    case Account::P_STATUS:
    {
        ...
        if (GetPropStatus(LoginStatus))
		{
			...
			case Account::LOGGED_IN:
				::Send (this->root->pInst, "CONNSTATUS ONLINE");
				::Send (this->root->pInst, "USERSTATUS ONLINE");
				GetPropSkypename(SkypeName);
				::Send (this->root->pInst, "CURRENTUSERHANDLE %s", (const char*)SkypeName);
				loggedIn = true;
				break;
			case Account::LOGGED_OUT:
				::Send (this->root->pInst, "USERSTATUS OFFLINE");
				::Send (this->root->pInst, "CONNSTATUS OFFLINE");
				loggedOut = true;
				break;
			case Account::CONNECTING_TO_P2P:
			case Account::CONNECTING_TO_SERVER :
				::Send (this->root->pInst, "CONNSTATUS CONNECTING");
				break;
			}
			return true;
		}
...
bool MyContact::DoChange(int prop, bool bError)
{
	...
	if (GetPropSkypename(name))
	{
		...
		case Contact::P_SKYPENAME:	// Doesn't make much sense, but for GET requests in Handler...
			::Send (root->pInst, "USER %s %s %s", (const char*)name, "HANDLE", (const char*)name);
			break;
		...
		case Contact::P_FULLNAME:
		{
			...
			if (bRet = GetPropFullname(fullname))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "FULLNAME", (const char*)fullname);
			break;
		}
		case Contact::P_BIRTHDAY:
		{
			...
			if (bRet = GetPropBirthday(birthday))
				::Send (root->pInst, "USER %s %s %d", (const char*)name, "BIRTHDAY", birthday);
			break;
		}
		case Contact::P_GENDER:
		{
			...
			if (bRet = GetPropGender(gender))
			{
				switch (gender)
				{
				case 1: pszGender="MALE"; break;
				case 2: pszGender="FEMALE"; break;
				default: pszGender="UNKNOWN"; break;
				}
				::Send (root->pInst, "USER %s %s %d", (const char*)name, "SEX", pszGender);
			}
			break;
		}
		case Contact::P_LANGUAGES:	
		{
			...
			if (bRet = GetPropLanguages(languages))
			{
				...
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "LANGUAGE", pLang);
				free (pszLang);
			}
			break;
		}
		case Contact::P_COUNTRY:	
		{
			...
			if (bRet = GetPropCountry(country))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "COUNTRY", (const char*)country);
			break;
		}
		case Contact::P_PROVINCE:
		{
			...
			if (bRet = GetPropProvince(province))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "PROVINCE", (const char*)province);
			break;
		}
		case Contact::P_CITY:
		{
			...
			if (bRet = GetPropCity(city))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "CITY", (const char*)city);
			break;
		}
		case Contact::P_PHONE_HOME:
		{
			...
			if (bRet = GetPropPhoneHome(phone))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "PHONE_HOME", (const char*)phone);
			break;
		}
		case Contact::P_PHONE_OFFICE:
		{
			...
			if (bRet = GetPropPhoneOffice(phone))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "PHONE_OFFICE", (const char*)phone);
			break;
		}
		case Contact::P_PHONE_MOBILE:
		{
			...
			if (bRet = GetPropPhoneMobile(phone))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "PHONE_MOBILE", (const char*)phone);
			break;
		}
		case Contact::P_HOMEPAGE:
		{
			...
			if (bRet = GetPropHomepage(homepage))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "HOMEPAGE", (const char*)homepage);
			break;
		}
		case Contact::P_ABOUT:
		{
			...
			if (bRet = GetPropAbout(about))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "ABOUT", (const char*)about);
			break;
		}
		case Contact::P_GIVEN_AUTHLEVEL:
		{
			...
			if (GetPropGivenAuthlevel(authlevel))
			{
				...
				case AUTHORIZED_BY_ME:
					::Send (root->pInst, "USER %s %s %s", (const char*)name, "ISAUTHORIZED", "TRUE");
					return true;
				case BLOCKED_BY_ME:
					::Send (root->pInst, "USER %s %s %s", (const char*)name, "ISBLOCKED", "TRUE");
					return true;
				}
			}
			break;
		...
		case Contact::P_LASTONLINE_TIMESTAMP:
		{
			...
			if (bRet = GetPropLastonlineTimestamp(laston))
				::Send (root->pInst, "USER %s %s %d", (const char*)name, "LASTONLINETIMESTAMP", laston);
			break;
		}
		case Contact::P_AVAILABILITY:
		{
			...
			if (bRet = GetPropAvailability(availability))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "ONLINESTATUS", MapAvailability(availability));
			break;
		}
		case Contact::P_RECEIVED_AUTHREQUEST:
		{
			...
			if (bRet = GetPropReceivedAuthrequest(authtxt))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "RECEIVEDAUTHREQUEST", (const char*)authtxt);
			break;
		}
		case Contact::P_MOOD_TEXT:
		{
			...
			if (bRet = GetPropMoodText (moodText))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "MOOD_TEXT", (const char*)moodText);
			break;
		}
		case Contact::P_RICH_MOOD_TEXT:
		{
			...
			if (bRet = GetPropRichMoodText (moodText))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "RICH_MOOD_TEXT", (const char*)moodText);
			break;
		}
		case Contact::P_TIMEZONE:
		{
			...
			if (bRet = GetPropTimezone (timezone))
				::Send (root->pInst, "USER %s %s %d", (const char*)name, "TIMEZONE", timezone);
			break;
		}
		case Contact::P_NROF_AUTHED_BUDDIES:
		{
			...
			if (bRet = GetPropNrofAuthedBuddies (nrauthed))
				::Send (root->pInst, "USER %s %s %d", (const char*)name, "NROF_AUTHED_BUDDIES", nrauthed);
			break;
		}
		case Contact::P_DISPLAYNAME:
		{
			...
			if (bRet = GetPropDisplayname (displayName))
				::Send (root->pInst, "USER %s %s %s", (const char*)name, "DISPLAYNAME", (const char*)displayName);
			break;
		}
		}
	}
        ...
        const Conversation::Ref& conversation)
{
	...
	if (MessageType == Message::POSTED_TEXT || MessageType == Message::POSTED_EMOTE)
	{
		...
		if (!strcmp((const char*)Author, pInst->pszUser))
		{
			...
			Send(pInst, "%sMESSAGE %d STATUS %s", pInst->iProtocol>=3?"CHAT":"", 
				message->getOID(), MapSendingStatus(status));
		
		}
		else
		{
			Send(pInst, "%sMESSAGE %d STATUS RECEIVED", pInst->iProtocol>=3?"CHAT":"", 
				message->getOID());
		}
	}
}; 
...
static void HandleMessage(IMOSAPI *pInst, char *pszMsg)
{
	...
	if (strcasecmp(pszCmd, "SEARCH") == 0)
	{
		...
		else if (strcasecmp(pszCmd, "ACTIVECALLS") == 0)
		{
			...
			strcasecmp(pszCmd, "ACTIVECHATS") == 0)
		{
			ConversationRefs cv;
	...
	if (strcasecmp(pszCmd, "GET") == 0)
	{
		...
		if (strcasecmp(pszCmd, "USER") == 0)
		{
			...
			else if (!strcasecmp (pszCmd, "BUDDYSTATUS"))
			{
				...
				user->IsMemberOfHardwiredGroup(ContactGroup::SKYPE_BUDDIES, bRes);
				if (bRes) status = 3;
				else
				{
					user->IsMemberOfHardwiredGroup(ContactGroup::CONTACTS_WAITING_MY_AUTHORIZATION, bRes);
					if (bRes) status = 2;
					else
					{
						user->IsMemberOfHardwiredGroup(ContactGroup::UNKNOWN_OR_PENDINGAUTH_BUDDIES , bRes);
						if (bRes) status = 0;
						else
						{
		...
		if (strcasecmp(pszCmd, "CURRENTUSERHANDLE") == 0)
		{
			...
			if (pInst->account->GetPropSkypename (handle))
		...
		if (strcasecmp(pszCmd, "USERSTATUS") == 0)
		{
			...
			pInst->account->GetPropAvailability(avail);
		...
		if (strcasecmp(pszCmd, "MESSAGE") == 0 || strcasecmp(pszCmd, "CHATMESSAGE") == 0)
		{
			...
			else if (!strcasecmp (pszCmd, "USERS"))
			{
				...
				if (message->GetPropConvoId(convo))
				{
					ParticipantRefs participantList;
					unsigned int i;
					SEString id, users;
	...
	if (strcasecmp(pszCmd, "SET") == 0)
	{
		...
		if (strcasecmp(pszCmd, "USER") == 0)
		{
			...
			if (strcasecmp(pszCmd, "BUDDYSTATUS") == 0)
			{
				...
				if (!user->SetBuddyStatus(bStatus))
					Send (pInst, "ERROR 519 Updating BUDDYSTATUS failed");
				else 
					Send (pInst, "USER %s BUDDYSTATUS %d", pszName, iStatus);
				return;
			}
			if (strcasecmp(pszCmd, "ISBLOCKED") == 0)
			{
				...
				if (pszCmd = strtok(NULL, " "))
				{
					if (!strcasecmp(pszCmd, "TRUE")) bBlocked=true;
				...
				if (!user->SetBlocked(bBlocked))
					...
					Send (pInst, "USER %s ISBLOCKED %s", pszName, pszCmd);
				return;
			}
			else
			{
				// Missing: ISAUTHORIZED
		...
		if (strcasecmp(pszCmd, "USERSTATUS") == 0)
		{
			...
			if (pInst->account->SetAvailability(avail))
		...
		if (strcasecmp(pszCmd, "PROFILE") == 0)
		{
			...
			{
				if (pInst->account->SetStrProperty(Account::P_MOOD_TEXT, pszCmd+10))
	...
	if (strcasecmp(pszCmd, "MESSAGE") == 0)
	{
		...
		if (pszCmd = strtok(NULL, " "))
		{
			bFoundContact = pInst->s->GetContact((pszUser = pszCmd), user);
		}
		...
		{
			if (conv->PostText(pszCmd, message))
			{
				Message::SENDING_STATUS status;
