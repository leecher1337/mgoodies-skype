...
void TemplateHTMLBuilder::buildHeadHistoryTemplate(IEView *view, IEVIEWEVENT *event) {
	DBVARIANT dbv;
	CONTACTINFO ci;
	char tempBase[1024];
	char tempStr[1024];
    ...
    TemplateMap *tmpm = TemplateMap::getTemplateMap((event->dwFlags & IEEF_RTL) ? protoSettings->getHistoryTemplateFilenameRtl() : protoSettings->getHistoryTemplateFilename());
	...
	szBase = Utils::UTF8Encode(tempBase);
	getUINs(event->hContact, szUINIn, szUINOut);
	if (protoSettings->getHistoryFlags() & Options::LOG_SHOW_NICKNAMES) {
		szNameOut = getEncodedContactName(NULL, szProto, szRealProto);
		szNameIn = getEncodedContactName(event->hContact, szProto, szRealProto);
	} else {
        szNameOut = Utils::dupString("&nbsp;");
	...
	Template *tmplt = TemplateMap::getTemplate((event->dwFlags & IEEF_RTL) ? protoSettings->getHistoryTemplateFilenameRtl() : protoSettings->getHistoryTemplateFilename(), "HTMLStart");
...
void TemplateHTMLBuilder::appendEventHistoryTemplate(IEView *view, IEVIEWEVENT *event) {
	DBVARIANT dbv;
	CONTACTINFO ci;
	HANDLE hRealContact;
	char *szRealProto = NULL;
...
//	char *szRealProto = getRealProto(event->hContact);
	...
	for (int eventIdx = 0; eventData!=NULL && (eventIdx < event->count || event->count==-1); eventData = eventData->next, eventIdx++) {
		...
		if (eventData->iType == IEED_EVENT_MESSAGE || eventData->iType == IEED_EVENT_STATUSCHANGE || eventData->iType == IEED_EVENT_FILE || eventData->iType == IEED_EVENT_URL) {
			...
			int isHistory = (eventData->time < (DWORD)getStartedTime() && (!(eventData->dwFlags & IEEDF_UNREAD) || eventData->dwFlags & IEEDF_SENT));
			int isGroupBreak = TRUE;
 		  	if ((protoSettings->getHistoryFlags() & Options::LOG_GROUP_MESSAGES) && eventData->dwFlags == LOWORD(getLastEventType())
			  && eventData->iType == IEED_EVENT_MESSAGE && HIWORD(getLastEventType()) == IEED_EVENT_MESSAGE
			  && (isSameDate(eventData->time, getLastEventTime()))
//			  && ((eventData->time < today) == (getLastEventTime() < today))
			  && (((eventData->time < (DWORD)startedTime) == (getLastEventTime() < (DWORD)startedTime)) || eventData->dwFlags & IEEDF_UNREAD)) {
			...
			if (eventData->iType == IEED_EVENT_MESSAGE) {
                if (isGrouping && (protoSettings->getHistoryFlags() & Options::LOG_GROUP_MESSAGES)) {
                    if (isGroupBreak) {
              		    tmpltName[1] = isHistory ? isSent ? "hMessageOutGroupStart" : "hMessageInGroupStart" : isSent ? "MessageOutGroupStart" : "MessageInGroupStart";
                   	} else {
                   		tmpltName[0] = isHistory ? isSent ? "hMessageOutGroupInner" : "hMessageInGroupInner" : isSent ? "MessageOutGroupInner" : "MessageInGroupInner";
			...
			for (int i=0;i<2;i++) {
				Template *tmplt;
				if (tmpltName[i] == NULL) continue;
				tmplt = TemplateMap::getTemplate((event->dwFlags & IEEF_RTL) ? protoSettings->getHistoryTemplateFilenameRtl() : protoSettings->getHistoryTemplateFilename(), tmpltName[i]);
				if (tmplt == NULL) continue;
				for (Token *token = tmplt->getTokens();token!=NULL;token=token->getNext()) {
					const char *tokenVal;
					tokenVal = NULL;
					switch (token->getType()) {
						...
						case Token::NAME:
							...
							break;
						case Token::TIME:
							if (protoSettings->getHistoryFlags() & Options::LOG_SHOW_TIME) {
	                            tokenVal = timestampToString(protoSettings->getHistoryFlags(), eventData->time, 1);
							} else {
                                tokenVal = "&nbsp;";
							}
							break;
						case Token::DATE:
							if (protoSettings->getHistoryFlags() & Options::LOG_SHOW_DATE) {
	                            tokenVal = timestampToString(protoSettings->getHistoryFlags(), eventData->time, 0);
							} else {
                                tokenVal = "&nbsp;";
							}
							break;
